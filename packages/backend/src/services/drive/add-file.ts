...
async function save(file: DriveFile, path: string, name: string, type: string, hash: string, size: number): Promise<DriveFile> {
	...
	if (config.objectStorage) {
		...
		const baseUrl = config.objectStorage.baseUrl
			|| `${ config.objectStorage.useSSL ? 'https' : 'http' }://${ config.objectStorage.endpoint }${ config.objectStorage.port ? `:${config.objectStorage.port}` : '' }/${ config.objectStorage.bucket }`;

		// for original
		const key = `${config.objectStorage.prefix}/${uuid()}${ext}`;
		...
		if (alts.webpublic) {
			webpublicKey = `${config.objectStorage.prefix}/webpublic-${uuid()}.${alts.webpublic.ext}`;
		...
		if (alts.thumbnail) {
			thumbnailKey = `${config.objectStorage.prefix}/thumbnail-${uuid()}.${alts.thumbnail.ext}`;
...
async function upload(key: string, stream: fs.ReadStream | Buffer, type: string, filename?: string) {
	...
	if (config.objectStorage.setPublicRead) params.ACL = 'public-read';
